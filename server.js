...
function throttleMessage(address, args, throttleTime = 20) {
  ...
  const key = (address === '/midi/cc' && args.length > 0) 
    ? `${address}-${args[0]}` 
    : address;
  ...
  if (messageThrottles[key] && now - messageThrottles[key].lastSent < throttleTime) {
...
udpPort.on("ready", () => {
  ...
  console.log(`- Sending to SuperCollider at: ${udpPort.options.remoteAddress}:${udpPort.options.remotePort}`);
});
...
udpPort.on("message", (oscMsg, timeTag, info) => {
  ...
  if (now - lastOscLogTime > OSC_LOG_INTERVAL) {
    console.log(`Received ${oscMessagesReceived} OSC messages in the last ${OSC_LOG_INTERVAL/1000} seconds`);
    lastOscLogTime = now;
...
wss.on('connection', (ws, req) => {
  ...
  console.log(`Web client connected from ${clientIp}. Total connections: ${connectionCount}`);
  ...
  ws.on('message', (message) => {
    ...
    try {
      ...
      if (data.type === 'osc' && data.address && Array.isArray(data.args)) {
        ...
        } else if (data.address === '/midi/noteon' || data.address === '/midi/noteoff') {
        ...
        if (shouldSend) {
          ...
          if (Math.random() < 0.01) { // Log only 1% of messages
            console.log('Sending OSC to SuperCollider:', data.address, processedArgs);
          }
  ...
  ws.on('close', () => {
    connectionCount--;
    console.log(`Web client disconnected. Remaining connections: ${connectionCount}`);
...
setInterval(() => {
  ...
  if (global.gc && memoryUsage.heapUsed > 200 * 1024 * 1024) { // 200 MB threshold
